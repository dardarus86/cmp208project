...
void SceneApp::InitPlayer()
{
	...
	player_fixture_def.density = 0.7f;
	...
	//player_body_->ApplyLinearImpulseToCenter(b2Vec2(10.0f, 0.0f), true);
...
{
	// ground dimensions
	gef::Vector4 ground_half_dimensions(1000.0f, 0.1f, 40.0f);
	b2Vec2 ground_start_position(990.0f, -0.3f);
...
{
	//20 roads. start 0, 20*20 = 400
	militaryroaddistancecounter = 0;
	//20roads, start at 400? 20*20 = 400
	samurairoaddistancecounter = 400;
	//20 roads, start at 440? 20*20 = 400
	apocroaddistancecounter = 800;
	...
	//military
	militaryroads[0] = LoadSceneAssets(platform_, miliroad_asset1);
	militaryroads[1] = LoadSceneAssets(platform_, miliroad_asset2);
	militaryroads[2] = LoadSceneAssets(platform_, miliroad_asset3);
	militaryroads[3] = LoadSceneAssets(platform_, miliroad_asset4);
	militaryroads[4] = LoadSceneAssets(platform_, miliroad_asset5);
	samurairoads[0] = LoadSceneAssets(platform_, samurairoad_asset1);
	samurairoads[1] = LoadSceneAssets(platform_, samurairoad_asset2);
	samurairoads[2] = LoadSceneAssets(platform_, samurairoad_asset3);
	samurairoads[3] = LoadSceneAssets(platform_, samurairoad_asset4);
	samurairoads[4] = LoadSceneAssets(platform_, samurairoad_asset5);
	apocroads[0] = LoadSceneAssets(platform_, apocroad_asset1);
	apocroads[1] = LoadSceneAssets(platform_, apocroad_asset2);
	apocroads[2] = LoadSceneAssets(platform_, apocroad_asset3);
	apocroads[3] = LoadSceneAssets(platform_, apocroad_asset4);
	apocroads[4] = LoadSceneAssets(platform_, apocroad_asset5);



	for (int i = 0; i < 20; i++)
	{
		int roadrandomnumber = rand() % 4;
		militaryroadmeshinstanceM44[i].SetIdentity();
		samurairoadmeshinstanceM44[i].SetIdentity();
		apocroadmeshinstanceM44[i].SetIdentity();

		samurairoadmeshinstance[i].set_mesh(GetMeshFromSceneAssets(samurairoads[roadrandomnumber]));
		militaryroadmeshinstance[i].set_mesh(GetMeshFromSceneAssets(militaryroads[roadrandomnumber]));
		apocroadmeshinstance[i].set_mesh(GetMeshFromSceneAssets(apocroads[roadrandomnumber]));
		militaryroadmeshinstanceV4[i] = gef::Vector4(militaryroaddistancecounter, 0, 0, 0);
		samurairoadmeshinstanceV4[i] = gef::Vector4(samurairoaddistancecounter, 0, 0, 0);
		apocroadmeshinstanceV4[i] = gef::Vector4(apocroaddistancecounter, 0, 0, 0);
		militaryroadmeshinstanceM44[i].SetTranslation(militaryroadmeshinstanceV4[i]);
		samurairoadmeshinstanceM44[i].SetTranslation(samurairoadmeshinstanceV4[i]);
		apocroadmeshinstanceM44[i].SetTranslation(apocroadmeshinstanceV4[i]);
		militaryroadmeshinstance[i].set_transform(militaryroadmeshinstanceM44[i]);
		samurairoadmeshinstance[i].set_transform(samurairoadmeshinstanceM44[i]);
		apocroadmeshinstance[i].set_transform(apocroadmeshinstanceM44[i]);
		

		militaryroaddistancecounter = militaryroaddistancecounter + 20;
		samurairoaddistancecounter = samurairoaddistancecounter + 20;
		apocroaddistancecounter = apocroaddistancecounter + 20;
	}
...
{
	militarybuildingdistancecounter = 20;
	samuraibuildingdistancecounter = 420;
	apocbuildingdistancecounter = 820;
	...
	militarybuildings[0] = LoadSceneAssets(platform_, milibuilding_asset1);
	militarybuildings[1] = LoadSceneAssets(platform_, milibuilding_asset2);
	militarybuildings[2] = LoadSceneAssets(platform_, milibuilding_asset3);
	militarybuildings[3] = LoadSceneAssets(platform_, milibuilding_asset4);
	militarybuildings[4] = LoadSceneAssets(platform_, milibuilding_asset5);
	samuraibuildings[0] = LoadSceneAssets(platform_, samuraibuilding_asset1);
	samuraibuildings[1] = LoadSceneAssets(platform_, samuraibuilding_asset2);
	samuraibuildings[2] = LoadSceneAssets(platform_, samuraibuilding_asset3);
	samuraibuildings[3] = LoadSceneAssets(platform_, samuraibuilding_asset4);
	samuraibuildings[4] = LoadSceneAssets(platform_, samuraibuilding_asset5);
	apocbuildings[0] = LoadSceneAssets(platform_, apocbuilding_asset1);
	apocbuildings[1] = LoadSceneAssets(platform_, apocbuilding_asset2);
	apocbuildings[2] = LoadSceneAssets(platform_, apocbuilding_asset3);
	apocbuildings[3] = LoadSceneAssets(platform_, apocbuilding_asset4);
	apocbuildings[4] = LoadSceneAssets(platform_, apocbuilding_asset5);

	for (int i = 0; i < 12; i++)
	{
	int buildingdrandomnumber = rand() % 4;
	int buildingrandomdisantcenumber = rand() % 10 +30;

	militarybuildingmeshinstance[i].set_mesh(GetMeshFromSceneAssets(militarybuildings[buildingdrandomnumber]));
	samuraibuildingmeshinstance[i].set_mesh(GetMeshFromSceneAssets(samuraibuildings[buildingdrandomnumber]));
	apocbuildingmeshinstance[i].set_mesh(GetMeshFromSceneAssets(apocbuildings[buildingdrandomnumber]));
	militarybuildingsmeshinstanceM44[i].SetIdentity();
	samuraibuildingsmeshinstanceM44[i].SetIdentity();
	apocbuildingsmeshinstanceM44[i].SetIdentity();
	militarybuildingsmeshinstanceV4[i] = gef::Vector4(militarybuildingdistancecounter -10, 0, 0, 0);
	samuraibuildingsmeshinstanceV4[i] = gef::Vector4(samuraibuildingdistancecounter, 0, 0, 0);
	apocbuildingsmeshinstanceV4[i] = gef::Vector4(apocbuildingdistancecounter, 0, 0, 0);
	militarybuildingsmeshinstanceM44[i].SetTranslation(militarybuildingsmeshinstanceV4[i]);
	samuraibuildingsmeshinstanceM44[i].SetTranslation(samuraibuildingsmeshinstanceV4[i]);
	apocbuildingsmeshinstanceM44[i].SetTranslation(apocbuildingsmeshinstanceV4[i]);
	militarybuildingmeshinstance[i].set_transform(militarybuildingsmeshinstanceM44[i]);
	samuraibuildingmeshinstance[i].set_transform(samuraibuildingsmeshinstanceM44[i]);
	apocbuildingmeshinstance[i].set_transform(apocbuildingsmeshinstanceM44[i]);

	militarybuildingdistancecounter = militarybuildingdistancecounter + buildingrandomdisantcenumber;
	samuraibuildingdistancecounter = samuraibuildingdistancecounter + buildingrandomdisantcenumber;
	apocbuildingdistancecounter = apocbuildingdistancecounter + buildingrandomdisantcenumber;
	}
...
{
	militaryobstacledistancecounter = 30;
	samuraiobstacledistancecounter = 430;
	apocobstacledistancecounter = 830;
	...
	const char* miliobstacle_asset1 = "military/obstacles/small.scn";
	const char* miliobstacle_asset2 = "military/obstacles/large.scn";
	const char* apocobstacle_asset1 = "apoc/obstacles/small.scn";
	const char* apocobstacle_asset2 = "apoc/obstacles/large.scn";
	militaryobstacle[0] = LoadSceneAssets(platform_, miliobstacle_asset1);
	militaryobstacle[1] = LoadSceneAssets(platform_, miliobstacle_asset2);
	samuraiobstacle[0] = LoadSceneAssets(platform_, samuraiobstacle_asset1);
	samuraiobstacle[1] = LoadSceneAssets(platform_, samuraiobstacle_asset2);
	apocobstacle[0] = LoadSceneAssets(platform_, apocobstacle_asset1);
	apocobstacle[1] = LoadSceneAssets(platform_, apocobstacle_asset2);

	for (int i = 0; i < 10; i++)
	{
		int obstaclerandomnumber = rand() % 2;
		int obstaclerandomdistance = rand() % 40 + 10;
		int obstacleupordown = rand() % 100;
		if (obstacleupordown > 50)
		{
			obstacleheight = 2.1;
		}
		else
		...
		militaryobstaclemeshinstance[i].set_mesh(GetMeshFromSceneAssets(militaryobstacle[obstaclerandomnumber]));
		samuraiobstaclemeshinstance[i].set_mesh(GetMeshFromSceneAssets(samuraiobstacle[obstaclerandomnumber]));
		apocoobstaclemeshinstance[i].set_mesh(GetMeshFromSceneAssets(apocobstacle[obstaclerandomnumber]));
		militaryobstaclemeshinstanceM44[i].SetIdentity();
		samuraiobstaclemeshinstanceM44[i].SetIdentity();
		apocobstaclemeshinstanceM44[i].SetIdentity();
		militaryobstaclemeshinstanceV4[i] = gef::Vector4(militaryobstacledistancecounter, obstacleheight, 0, 0);
		samuraiobstaclemeshinstanceV4[i] = gef::Vector4(samuraiobstacledistancecounter, obstacleheight, 0, 0);
		apocobstaclemeshinstanceV4[i] = gef::Vector4(apocobstacledistancecounter, obstacleheight, 0, 0);
		militaryobstaclemeshinstanceM44[i].SetTranslation(militaryobstaclemeshinstanceV4[i]);
		samuraiobstaclemeshinstanceM44[i].SetTranslation(samuraiobstaclemeshinstanceV4[i]);
		apocobstaclemeshinstanceM44[i].SetTranslation(apocobstaclemeshinstanceV4[i]);
		militaryobstaclemeshinstance[i].set_transform(militaryobstaclemeshinstanceM44[i]);
		samuraiobstaclemeshinstance[i].set_transform(samuraiobstaclemeshinstanceM44[i]);
		apocoobstaclemeshinstance[i].set_transform(apocobstaclemeshinstanceM44[i]);

		militaryobstaclepositionb2vec2[i] = b2Vec2(militaryobstacledistancecounter, obstacleheight);
		samuraiobstaclepositionb2vec2[i] = b2Vec2(samuraiobstacledistancecounter, obstacleheight);
		apocobstaclepositionb2vec2[i] = b2Vec2(apocobstacledistancecounter, obstacleheight);
		militarybodydef[i].type = b2_staticBody;
		samuraibodydef[i].type = b2_staticBody;
		apocbodydef[i].type = b2_staticBody;
		militarybodydef[i].position = militaryobstaclepositionb2vec2[i];
		samuraibodydef[i].position = samuraiobstaclepositionb2vec2[i];
		apocbodydef[i].position = apocobstaclepositionb2vec2[i];
		militaryobstacleB2B[i] = world_->CreateBody(&militarybodydef[i]);
		samuraiobstacleB2B[i] = world_->CreateBody(&samuraibodydef[i]);
		apocobstacleB2B[i] = world_->CreateBody(&apocbodydef[i]);
		militaryshape[i].SetAsBox(1.0,1.8);
		samuraishape[i].SetAsBox(0.1,0.1);
		apocshape[i].SetAsBox(0.1, 0.1);
		militaryobstaclefixturedef[i].shape = &militaryshape[i];
		samuraiobstaclefixturedef[i].shape = &militaryshape[i];
		apocobstaclefixturedef[i].shape = &apocshape[i];
		militaryobstacleB2B[i]->CreateFixture(&militaryobstaclefixturedef[i]);
		samuraiobstacleB2B[i]->CreateFixture(&samuraiobstaclefixturedef[i]);
		apocobstacleB2B[i]->CreateFixture(&apocobstaclefixturedef[i]);


		militaryobstacledistancecounter = militaryobstacledistancecounter + obstaclerandomdistance;
		samuraiobstacledistancecounter = samuraiobstacledistancecounter + obstaclerandomdistance;
		apocobstacledistancecounter = apocobstacledistancecounter + obstaclerandomdistance;
...
bool SceneApp::Update(float frame_time)
{
	...
	if (input_manager_->keyboard()->IsKeyDown(gef::Keyboard::KC_S))
	{
		...
		if (issliding_)
		{
			player_shape.SetAsBox(0.1f, 0.05f);
...
void SceneApp::Render()
{
	...
	for (int i = 0; i < 20; i++)
	{
		renderer_3d_->DrawMesh(militaryroadmeshinstance[i]);
		renderer_3d_->DrawMesh(samurairoadmeshinstance[i]);
		renderer_3d_->DrawMesh(apocroadmeshinstance[i]);
	}

	for (int i = 0; i < 12; i++)
	{
		renderer_3d_->DrawMesh(militarybuildingmeshinstance[i]);
		renderer_3d_->DrawMesh(samuraibuildingmeshinstance[i]);
		renderer_3d_->DrawMesh(apocbuildingmeshinstance[i]);
	}

	for (int i = 0; i < 10; i++)
	{
		renderer_3d_->DrawMesh(militaryobstaclemeshinstance[i]);
		renderer_3d_->DrawMesh(samuraiobstaclemeshinstance[i]);
		renderer_3d_->DrawMesh(apocoobstaclemeshinstance[i]);
	}
